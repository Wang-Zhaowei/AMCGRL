...
def encode_smiles(line, smi_ch_ind, MAX_SMI_LEN=100):
...
def load_drug_feat(seq_file, moles):
	seq_dict = {}
	with open(seq_file, 'r') as rf:
		seq = ''
		for line in rf:
			...
			if line[0] == '>':
				...
				seq = line.upper()
				string = encode_smiles(seq, CHARISOSMISET, 100)
				seq_dict[moles.index(name)] = string
	return seq_dict
...
def dataset(x_sim_mat, y_sim_mat, pairs, x_feat_dict, y_feat_dict):
	...
	posi_index = np.where(pairs[:, -1] == 1)
	posi_pairs = pairs[posi_index][:, :2]
	G = torch.tensor(posi_pairs.T, dtype=torch.long)
	G = to_undirected(G)
	dataset['Inter'] = G
