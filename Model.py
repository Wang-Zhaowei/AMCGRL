...
class multihead_cross_attention(nn.Module):
    def __init__(self, d_model, n_heads):
        ...
        self.W_Q = nn.Conv1d(d_model, self.d_k*n_heads, kernel_size=1, stride=1, padding=0)
        self.W_K = nn.Conv1d(d_model, self.d_k*n_heads, kernel_size=1, stride=1, padding=0)
        self.W_V = nn.Conv1d(d_model, self.d_k*n_heads, kernel_size=1, stride=1, padding=0)
...
class cross_domain_decoder(nn.Module):
    def __init__(self, d_model, n_heads):
        super().__init__()
        self.decoder = multihead_cross_attention(d_model, n_heads)
...
class hierarchical_mutual_attention(nn.Module):
    def __init__(self, d_model, n_heads):
        super().__init__()
        self.HMCA = inter_molecule_attention(d_model, n_heads)
...
class AMCGRL(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.embedding_dim = args.embedding_dim
        self.intra_enc = intra_domain_encoder(args.embedding_dim, args.att_g)
        self.inter_enc = inter_domain_encoder(args.embedding_dim, args.att_g)
        self.decoder = cross_domain_decoder(args.embedding_dim, args.att_c)
        self.fusion = hierarchical_mutual_attention(args.embedding_dim * 2, args.att_c)

    def forward(self, n_x, n_y, data):
        torch.manual_seed(1)
        x_d = data['X']['attribute'].shape[1]
        f_x = nn.Linear(x_d, self.embedding_dim)(data['X']['attribute'])
        f_x_intra = self.intra_enc('X', f_x, data)
        y_d = data['Y']['attribute'].shape[1]
        f_y = nn.Linear(y_d, self.embedding_dim)(data['Y']['attribute'])
...
def feature_representation(model,n_x, n_y, dataset, args):
    model.cuda()
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    model = train(model,n_x,n_y, dataset, optimizer, args.epochs)
    model.eval()
    with torch.no_grad():
...
def train(model,n_x,n_y, train_data, optimizer, epochs):
    model.train()
